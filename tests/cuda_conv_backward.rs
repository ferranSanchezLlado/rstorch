//! CUDA conv/pool **backward** against the CPU reference, across the
//! stride/padding/dilation grid.
//!
//! The in-crate CUDA test (`conv_and_pool_backward_execute_on_cuda`) pins
...
fn assert_close(what: &str, case: usize, cpu: &[f32], cuda: &[f32]) {
    assert_eq!(cpu.len(), cuda.len(), "case {case}: {what} length differs");
    for (i, (want, got)) in cpu.iter().zip(cuda).enumerate() {
        let tolerance = 1e-4 * want.abs().max(1.0);
        assert!(
            (want - got).abs() <= tolerance,
            "case {case}: {what}[{i}] = {got} on CUDA, {want} on the CPU reference"
        );
    }
}
...
fn conv2d_backward_matches_the_cpu_across_stride_padding_and_dilation() {
    ...
    for (index, case) in cases.iter().enumerate() {
        ...
        let (cuda_input, cuda_weight) = conv_grads(&CUDA, case, &x, &w, &scale);
        assert_close("input grad", index, &cpu_input, &cuda_input);
        assert_close("weight grad", index, &cpu_weight, &cuda_weight);
    }
}
...
fn pool2d_backward_matches_the_cpu_across_stride_and_padding() {
    ...
    for (kernel, stride, padding) in [
        ...
        for max_pool in [true, false] {
            ...
            assert_close(what, 0, &grads_on(&Device::Cpu), &grads_on(&CUDA));
        }
    }
}
