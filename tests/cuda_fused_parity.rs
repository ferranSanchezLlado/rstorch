...
fn assert_close(what: &str, cpu: &[f32], cuda: &[f32]) {
    assert_eq!(cpu.len(), cuda.len(), "{what}: length differs");
    for (i, (want, got)) in cpu.iter().zip(cuda).enumerate() {
        let tolerance = 2e-4 * want.abs().max(1.0);
        assert!(
            (want - got).abs() <= tolerance,
            "{what}[{i}] = {got} on CUDA, {want} on the CPU reference"
        );
    }
}
...
/// `BatchNorm` keeps running buffers, so a CUDA divergence accumulates across
