...
//!    `test-tools/gen_reference_vectors.py` (torch **2.13.0**, pinned and asserted)
...
mod vectors {
    ...
    /// Where `test-tools/gen_reference_vectors.py` writes, relative to the crate
    /// root.
    pub const PATH: &str = "tests/data/reference_vectors.json";

    const REGENERATE: &str =
        "regenerate with a pinned PyTorch:  python3 test-tools/gen_reference_vectors.py";
    ...
    for &i in &skip {
        assert!(
            ...
            "torch's fully-masked softmax is no longer NaN at {i} ({}); the divergence \
             note in test-tools/gen_reference_vectors.py needs revisiting",
            soft[i]
        );
        assert!(
            logsoft[i].is_nan(),
...
// ===========================================================================
// 4. release-1.0 math/activation/shape ops — finite-difference coverage
// ===========================================================================
