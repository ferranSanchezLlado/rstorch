//! Synchronized CPU-vs-wgpu-vs-Metal kernel-family benchmarks.
//!
//! Every operation ends in a host read so asynchronous backends are measured
//! through completion rather than only through command submission.
...
fn bench_device(c: &mut Criterion, device: Device) {
    let mut group = c.benchmark_group(format!("wgpu_ops/{device}"));
    ...
    let f16_supported = Tensor::from_vec(vec![f16::ONE], [1], &device).is_ok();
    if f16_supported {
