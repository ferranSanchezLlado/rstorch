...
fn apply_op_f32(left: f32, right: f32, op: u32) -> f32 {
    switch op {
...
fn binary_f32_kernel(@builtin(global_invocation_id) id: vec3<u32>) {
    let gid = id.x;
    if gid >= params.len {
        return;
    }
    out[gid] = apply_op_f32(lhs[gid], rhs_values[gid], params.op);
}

@compute @workgroup_size(64)
fn scalar_f32_kernel(@builtin(global_invocation_id) id: vec3<u32>) {
    let gid = id.x;
    if gid >= params.len {
        return;
    }
    out[gid] = apply_op_f32(lhs[gid], params.rhs, params.op);
}

@compute @workgroup_size(64)
fn matmul_f32_kernel(@builtin(global_invocation_id) id: vec3<u32>) {
    let gid = id.x;
...
@compute @workgroup_size(64)
fn sum_f32_kernel(@builtin(global_invocation_id) id: vec3<u32>) {
    if id.x != 0u {
        return;
    }
