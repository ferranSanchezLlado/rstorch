    ...
    MultiHeadAttention<EMBED, HEADS, E, P>
{
        ...
        C: AttentionContext<EMBED, HEADS, E, P>,
    {
        ...
        shape.push(EMBED);
        let merged = checked_wrap::<C::Output>(
            merged.reshape(shape)?,
            Arc::clone(context.binding()),
            "typed::nn::MultiHeadAttention::project_output",
        )?;
    ...
    fn explicit_mask_and_dynamic_width_errors_are_early() {
        ...
        let unbroadcast =
            Tensor4::<DYN, 2, DYN, DYN, bool>::from_vec(vec![false; 4], [1, 2, 1, 2], &ctx)
                .unwrap();
        assert!(matches!(
            attention.attend(&input, Some(&unbroadcast), Mode::EVAL),
            Err(Error::ShapeMismatch { .. })
        ));
        let wrong = Tensor3::<DYN, DYN, DYN>::from_vec(vec![0.0; 6], [1, 2, 3], &ctx).unwrap();
        assert!(matches!(
            attention.attend(&wrong, None, Mode::EVAL),
            Err(Error::ShapeMismatch { .. })
        ));
